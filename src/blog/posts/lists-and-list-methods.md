...
date: "2026-07-09"
description: "A categorical survey of Cons-Lists and emerging list methods"
...
2. **Identity**: This condition states that for each object in $C$ we have a unique morphism from that object to itself, which returns itself and is equivalent to essentially doing nothing to the object. In diagrams, these morphisms are often omitted but are always implicitly present in any valid category.
	This identity morphism must satisfy:
$$
...
![arbitrary category](/blog/lists-and-list-methods/category.png "An arbitrary category with three objects and 3 morphisms and identity morphisms")

We must note that we haven't really put any restrictions on what objects are or even what morphisms are. I quite appreciate this about category theory because as long as I can define a structure that satisfies these constraints (though that's not always very easy to do), I can call that structure a category.
...
![functor example](/blog/lists-and-list-methods/functor.png "Commutative diagram of a functor from a category C to D ")
...
Finally, to put this all into context, we can think of `List` as an (endo)functor on types. So every type `a` under the action of this functor gets mapped to the type `List a`. It's quite simple to think of object mappings in this manner; some examples include:
...
But a more interesting aspect of List being a functor is trying to identify its `fmap`. To recap, the signature of this function lifting function is
```
fmap :: (a -> b) -> (F a -> F b)
```
For the specific instance of the List functor, our signature becomes:
```
...
Here is my best attempt at defining a monad while tiptoeing between boring pragmatism and abstract nonsense. A monad $T: C \to C$, is an *endofunctor* (as encountered above), equipped with the following *natural transformations*:
$$
\eta : Id \to T
$$
$$
...
![monad laws](/blog/lists-and-list-methods/monad_laws.png "Monad laws depicted by commutative diagrams for an arbitrary monad T")
...
![free monoid construction](/blog/lists-and-list-methods/free_monoid.png "Commutative diagram showing the univseral property of the free monoid")
...
![F-algebra](/blog/lists-and-list-methods/f_algebra.png "Commutative diagram for a morphism between two F-algebras")
...
```
And through this, we could define and evaluate an expression like:
```
-(2 + 3) * (5 * 4)
```
as
...
![catamorphism](/blog/lists-and-list-methods/catamorphism.png "Commutative diagram showing a catamorphism for the ListF algebra")
...
![anamorphism](/blog/lists-and-list-methods/anamorphism.png "Commutative diagram showing an anamorphism for the ListF algebra")
